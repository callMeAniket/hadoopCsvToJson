...
object nested_json extends App{
  ...
  val output_df=joined_df.groupBy(department_df("id"),department_df("name")).agg(collect_list(employee_df("first_name")).as("employee name"))
  output_df.show()
  output_df.write.mode("overwrite").format("json").save("/Users/aniketsharma/Documents/spark_json/nested_dept_employee.json")
}
